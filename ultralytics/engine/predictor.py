...
from ultralytics.data.multimodal import get_multimodal_settings, split_modalities
...
class BasePredictor:
    ...
    def setup_source(self, source):
        ...
        if not getattr(self, "stream", True) and (
            self.source_type.stream or self.source_type.screenshot or len(self.dataset) > 1000 or any(getattr(self.dataset, "video_flag", [False]))
        ):
        ...
        with self._lock:
            ...
            for self.batch in self.dataset:
                ...
                for i in range(n):
                    ...
                    visible_im = im
                    infrared_im = None
                    if im.shape[1] == 6 and multimodal["input_modality"] == "multimodal":
                        visible_np, infrared_np = split_modalities(
                            im[i].permute(1, 2, 0).detach().cpu().numpy(), multimodal["channel_order"]
                        )
                        visible_im = visible_np.transpose(2, 0, 1)[None]
                        infrared_im = infrared_np.transpose(2, 0, 1)[None]

                    if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:
                        s[i] += self.write_results(i, Path(paths[i]), visible_im, s, self.results)
                    if self.ir_results is not None and infrared_im is not None:
                        if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:
                            self.write_results(i, Path(paths[i][:-4] + "ir" + paths[i][-4:]), infrared_im, s, self.ir_results)
